...
// ─── Health Modal ─────────────────────────────────────────────────────────────
const HealthModal = ({ onClose, healthGoals, setHealthGoals }) => {
...
const PantryInventory = () => {
  ...
  // ── Health goals (persisted to localStorage) ──
  const [healthGoals, setHealthGoalsState] = useState(() => {
    try {
      const stored = localStorage.getItem('epicurean-health-goals');
      return stored ? JSON.parse(stored) : { calories: '', fitnessGoal: 'General Health', glucoseTarget: '' };
    } catch {
      return { calories: '', fitnessGoal: 'General Health', glucoseTarget: '' };
    }
  });

  const setHealthGoals = (goals) => {
    setHealthGoalsState(goals);
    try {
      localStorage.setItem('epicurean-health-goals', JSON.stringify(goals));
    } catch {}
  };
