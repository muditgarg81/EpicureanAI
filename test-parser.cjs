...
const parseDetailedRecipe = (rawText) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Check if line is a section header (using loose but precise regexes)
    if (
...
async function testParse() {
  const { data, error } = await supabase.from('recipes').select('id, dish_name, detailed_recipe');
  if (error) {
    console.error(error);
    return;
  }
  ...
  for (const r of data) {
    ...
    if (!parsed) {
      failed++;
      console.log(`FAILED ID ${r.id}: ${r.dish_name}`);
      console.log(`Raw snippet:`, JSON.stringify(r.detailed_recipe?.slice(0, 300)));
    }
  }
  console.log(`Failed to parse: ${failed} / ${data.length}`);
}
